...
const fallBack = (candidateUsers: UserProfile[]): CompatibilityResult[] => {
...
const generateBatchCompatibilityPrompt = (
  currentUser: UserProfile,
  candidateUsers: UserProfile[]
): string => {
  return `
  Phân tích độ tương thích của User A với ${
  ...
  - Khoảng tuổi mong muốn: ${
    currentUser.age_range
      ? `${currentUser.age_range[0]}-${currentUser.age_range[1]}`
      : "Không có thông tin"
  }
  - Khoảng cách tối đa (km): ${currentUser.distance || "Không có thông tin"}
  - Đã xác thực: ${
    typeof currentUser.is_verified === "boolean"
  ...
  - Khoảng tuổi mong muốn: ${
    user.age_range
      ? `${user.age_range[0]}-${user.age_range[1]}`
      : "Không có thông tin"
  }
...
export const generateCompatibilityScores = async (
  currentUser: UserProfile,
  candidateUsers: UserProfile[]
): Promise<CompatibilityResult[]> => {
  if (apiKeys.length === 0) {
    console.error(
      "Lỗi: Không có khóa API Gemini nào được định cấu hình trong các biến môi trường (GEMINI_API_KEY1-8)."
    );
    return fallBack(candidateUsers);
  }
  try {
    const batchSize = 1;
    ...
    const batchPromises = batches.map(async (batch) => {
      ...
      if (!apiKey) {
        console.error(
          "Không có API key nào của Gemini để sử dụng. Bỏ qua batch này."
        );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Lỗi API Gemini: ${data?.error?.message || "Unknown error"}`
        );
      }

      if (!data.candidates || !data.candidates[0]?.content?.parts?.[0]?.text) {
        console.error(
          "Định dạng phản hồi API Gemini không mong muốn:",
          JSON.stringify(data)
        );
        return [];
      }
    ...
    const allResults = settledResults
      .filter(
        (result): result is PromiseFulfilledResult<CompatibilityResult[]> =>
